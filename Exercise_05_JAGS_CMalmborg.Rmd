...
Older texts will advise that you thin your posterior samples to account for autocorrelation. Current thinking is that this is unnessesary unless you need to reduce the size of the output you save. Intuitively, setting “thin” to 10 retains only every 10th sample for further analysis, etc.
```{r}
...
plot(x1,y, col='slateblue1', pch=8)
abline(a=jags.summ$statistics["b[1]","Mean"], 
       b=jags.summ$statistics["b[2]","Mean"],
       col='navy')
```

* Describe and explain the parameter covariances that you observe in the pairs plot and parameter correlation matrix.

> We can see from the parameter pairs plot and correlation matrix that b1 and b2 are negatively correlated. The pairs plot for these parameters have linear relationships with negative slopes for b1 and b2. This is confirmed in the correlation matrix which shows ~ -0.89 correlation (close to -1) between b1 and b2, they are fairly well-correlated. Neither b1 nor b2 are correlated with S.
...
```{r}
cr.summ<-summary(lm(y ~ x1))

#Model intercepts:
print ('Classical regression intercept:')
cr.summ$coefficients["(Intercept)",'Estimate']

print('Bayesian regression intercept:')
jags.summ$statistics["b[1]",'Mean']


#Model slopes:
print('Classical regression slope:')
cr.summ$coefficients["x1",'Estimate']

print('Bayesian regression slope:')
jags.summ$statistics["b[2]",'Mean']


#Model uncertainties:
print('Classical regression uncertainty:')
cr.summ$sigma

print('Bayesian regression uncertainty:')
S<-jags.summ$statistics["S",'Mean']
1/sqrt(S)
```
...
## multivariate Normal prior on vector of regression params
  b ~ dmnorm(b0,Vb) 
...
## specify priors
# regression b means
data$b0 <- as.vector(c(0,0,0,0))
data$Vb <- solve(diag(10000,4))   
# error prior n/2
data$s1 <- 0.1
#error prior SS/2
data$s2 <- 0.1
...
#Look at pairs plots and correlation matrix:
out <- as.matrix(jags.burn)
pairs(out)
