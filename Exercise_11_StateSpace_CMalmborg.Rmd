...
1.  Write out and run the JAGS code for the exponential growth model.  Include the JAGS code and standard MCMC diagnostics in your lab report. 
```{r}
library(rjags)

#load moose data:
dat<-read.table("data/alcesdata.txt", header=T)
...
#data for model:
data <- list(y=log(dat$density), N=length(dat$year),
             s1=0.001,s2=0.001,
             t1=0.001, t2=0.001,
             r0=1, vr=0.001,
#             k0=300, vk=0.001,
...
j.model <- jags.model (file = textConnection(mooses),
                             data = data,
                             n.chains = 3)

jags.out <- coda.samples(model = j.model,
                         variable.names = c("sigma","r","tau","x",
                                            "y[18]","y[19]","y[20]",
                                            "y[21]","y[44]"),
                         n.iter = 50000)
```
...
```{r}
#plot log scale:
Xs = out[,grep("x",colnames(out))]

plot(dat$year,log(dat$density),
     xlab="Year",ylab="Moose Population Density",
     col='brown',pch=20, main="Log Scale Plot",
     ylim=c(0,6))

ci <- apply(Xs,2,quantile,c(0.025,0.5,0.975))

lines(dat$year,ci[2,],col=3,lwd=2)  ## median model
lines(dat$year,ci[1,],col=3,lty=2) ## model CI
lines(dat$year,ci[3,],col=3,lty=2)


#plot linear scale:
plot(dat$year,dat$density,
     xlab="Year",ylab="Moose Population Density",
     col='brown',pch=20, main="Linear Scale Plot",
     ylim=c(-1,400))

ci <- apply(exp(Xs),2,quantile,c(0.025,0.5,0.975))
...
#use func density() instead of hist for all y's you need
Ys<-out[,grep("y",colnames(out))]

ds<-list()
for (i in 1:ncol(Ys)){
  ds[[i]]<-density(Ys[,i])
  plot(ds[[i]], main=as.character(colnames(Ys)[i]))
}
```

```{r}
#PARAMETER DENISTY PLOTS

outpar<-cbind(r=out[,grep("r",colnames(out))],
              sigma=out[,grep("sigma",colnames(out))],
              tau=out[,grep("tau",colnames(out))])

ds<-list()
for (i in 1:ncol(outpar)){
  ds[[i]]<-density(outpar[,i])
  plot(ds[[i]], main=as.character(colnames(outpar)[i]))
}
...
inits <- list(k=300,r=0.1,sigma=13,tau=300,x=colMeans(Xs))

j.model.2 <- jags.model (file = textConnection(meeses),
                             data = data,
                             inits = inits,
                             n.chains = 3)

jags.out.2 <- coda.samples(model = j.model.2,
                         variable.names = c("sigma","r","tau","k","x",
                                            "y[18]","y[19]","y[20]",
                                            "y[21]","y[44]"),
                         n.iter = 100000)
```
```{r}
# plot(jags.out.2)
# effectiveSize(jags.out.2)
# gelman.plot(jags.out.2)

burnin=15000
jags.burn.2 <-window(jags.out.2,start=burnin)
#plot(jags.burn.2)

out.2<-as.matrix(jags.burn.2)
summ2<-summary(jags.out.2)
```

```{r}
#plot log scale:
Xs.2 = out[,grep("x",colnames(out.2))]
...
ci.2 <- apply(Xs.2,2,quantile,c(0.025,0.5,0.975))

lines(dat$year,ci.2[2,],col=3,lwd=2)  ## median model
lines(dat$year,ci.2[1,],col=3,lty=2) ## model CI
lines(dat$year,ci.2[3,],col=3,lty=2)
...
ci.2 <- apply(exp(Xs.2),2,quantile,c(0.025,0.5,0.975))
...
#use func density() instead of hist for all y's you need
Ys.2<-out.2[,grep("y",colnames(out.2))]

ds<-list()
for (i in 1:ncol(Ys.2)){
  ds[[i]]<-density(Ys.2[,i])
  plot(ds[[i]], main=as.character(colnames(Ys.2)[i]))
}
...
outpar.2<-cbind(r=out.2[,grep("r",colnames(out.2))],
                #k=out.2[,grep("k",colnames(out.2))],
                sigma=out.2[,grep("sigma",colnames(out.2))],
                tau=out.2[,grep("tau",colnames(out.2))])

ds<-list()
for (i in 1:ncol(outpar.2)){
  ds[[i]]<-density(outpar.2[,i])
  plot(ds[[i]], main=as.character(colnames(outpar.2)[i]))
}

kseq=1:1000
plot(kseq,dlnorm(kseq,data$k0,1/sqrt(data$vk)), main="k densities", type="l")
lines(density(out.2[,grep("k",colnames(out.2))]),col="red")
```
...
```{r}
#do the DICs:
DIC.grow<-dic.samples(j.model, n.iter=10000)
DIC.rick<-dic.samples(j.model.2, n.iter=10000)

modelcompare<-data.frame(matrix(ncol = 2, nrow = 1))
modelcompare[1,]<-c(sum(DIC.grow$deviance,DIC.grow$penalty),
                    sum(DIC.rick$deviance,DIC.rick$penalty))

#rownames(modelcompare)<-c("DIC","Sigma")
colnames(modelcompare)<-c("Growth Model DIC", "Ricker Model DIC")
...
#GROW MODEL:
#print(summ1)
grow<-as.matrix(summ1$statistics)
gq<-as.matrix(summ1$quantiles)

#RICKER MODEL:
#print(summ2)
rick<-as.matrix(summ2$statistics)
rq<-as.matrix(summ2$quantiles)

k<-as.matrix(cbind(NA,rick["k","Mean"]))
rs<-as.matrix(cbind(grow["r","Mean"],rick["r","Mean"]))
sigmas<-as.matrix(cbind(grow["sigma","Mean"],rick["sigma","Mean"]))
taus<-as.matrix(cbind(grow["tau","Mean"],rick["sigma","Mean"]))
# y<-as.matrix(cbind(grow[grep("y",rownames(grow)),"Mean"],
#          rick[grep("y",rownames(rick)),"Mean"]))

kq<-as.matrix(cbind(NA,rq["k","97.5%"]))
rsq<-as.matrix(cbind(gq["r","97.5%"],rq["r","97.5%"]))
sigmasq<-as.matrix(cbind(gq["sigma","97.5%"],rq["sigma","97.5%"]))
tausq<-as.matrix(cbind(gq["tau","97.5%"],rq["sigma","97.5%"]))
# yq<-as.matrix(cbind(gq[grep("y",rownames(gq)),"97.5%"],
#           rq[grep("y",rownames(rq)),"97.5%"]))

paramsum<-rbind(k,kq,
                rs,rsq,
                sigmas,sigmasq,
                taus,tausq)
rownames(paramsum)<-c("K","K 95% CI","R","R 95% CI",
                    "Sigma"," Sigma 95% CI",
                    "Tau","Tau 95% CI")
colnames(paramsum)<-c("Grow","Ricker")

library(knitr)
kable(paramsum, caption="Summary Table")
