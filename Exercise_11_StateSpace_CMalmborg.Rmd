...
#data for model:
data <- list(x=dat$density, N=length(dat$year),
             s1=0.001,s2=0.001,
             t1=100, t2=0.001,
             r0=1, vr=0.001,
#             k0=300, vk=0.001,
...
niter <- nrow(out)
nt <- length(dat$year)
xpred <- seq(min(dat$year),max(dat$year),length=length(dat$year))
npred <- length(xpred)
...
for(t in 1:nt){
  for(i in 1:niter){
    ...
    ycred[i,] <- Ex[i,]
    ypred[i,t]<-rnorm(1,Ex[i,t],1/sqrt(outs[i,2]))
  }
}
...
sel.t <- grep("tau",colnames(jags.mat))
sel.y <-grep("y",colnames(jags.mat))
louts <- jags.mat[,c(sel.r,sel.sig,sel.t)]
lmu <- jags.mat[,sel.mu]
lys <- jags.mat[,sel.y]
...
for(t in 1:nt){
  for(i in 1:niter){
    Ey <- exp(lmu[i,t] + louts[i,1])
    Ex[i,t] <- as.numeric(Ey)
    ycred[i,] <- Ex[i,]
    ypred[i,t]<-rlnorm(1,Ex[i,t],1/sqrt(louts[i,2]))
  }
}
