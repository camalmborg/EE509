...
j.model   <- jags.model (file = textConnection(mosquito_bite),
                             data = data,
                             n.chains = 3)
...
niter <- nrow(out)
xpred <- seq(min(dat$time),max(dat$time),length=30)
npred <- length(xpred)
...
```{r}
summ<-summary(jags.out)
summ

sigglywiggly<-1/sqrt(summ$statistics["sigma",1])
tautau<-1/sqrt(summ$statistics["tau.t",1])
totalerror<-sum(sigglywiggly,tautau)
