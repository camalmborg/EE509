...
> R represents non-numbers with NaN and infinite values with -Inf and Inf.

2.	Why are we multiplying the results of the atan and atan2 functions by 180/pi? 

> This is how we obtain arctan in degrees since atan returns radians measurements.

3.	How is the atan2 function different from the atan function? 

> atan2 returns a value between pi and -pi, atan returns a value between pi/2 and -pi/2.

4.	What is the difference between log and log10?

> log() is the natural logarithm, log10() is the log base 
...
> sum() gives the sum of numbers listed, cumsum() gives the cumulative sum, adding the next number to the sum of the first two numbers for all the numbers in a group.
...
```{r}
#save frogs file:
write.table(dat,"my_frogs.txt",sep="|")

#reload:
dat2 <- read.table("my_frogs.txt",sep='|',header=T)
```
...
```{r}
heroes <- c('Wonder Woman', 'Captain Marvel', 'Black Widow', 'Storm')
```
...
plot(log(x), pch=20, col='chartreuse')
# the return of terrible chartreuse plots!
```
   
   b. $e^{-x}$
```{r}
plot(exp(-x), pch=20, col='darkorchid3')
```
   
  20.	Make a barplot of the median number of frogs grouped by whether they have spots or not.
```{r}
hist(tapply(dat$frogs,dat$spots,median))

```
  
  21.	Plot a histogram of blue frogs
```{r}
hist(dat$frogs[dat$color=="blue"],xlab="Blue Frogs",ylab="Frequency",col="cornflowerblue")
...
```{r}
reddat <- dat[dat$color=='red',]
tadlm <- lm(tadpoles ~ frogs, data=reddat)
summary(tadlm)
```
  
  b.	Make a scatter plot of this data that includes the regression line
```{r}
plot(reddat$frogs,reddat$tadpoles,
     pch=16, col='darkgreen', main='Ribbit',
     xlab='Frogs', ylab='Tadpoles')
abline(tadlm, col='black')
```
...
#simple models:
tflm <- lm(tadpoles ~ frogs, data=dat)
tslm <- lm(tadpoles ~ spots, data=dat)
tclm <- lm(tadpoles ~ color, data=dat)

tfslm <- lm(tadpoles ~ frogs+spots, data=dat)
tfclm <- lm(tadpoles ~ frogs+color, data=dat)
tfsclm <- lm(tadpoles ~ frogs+spots+color, data=dat)

tfsslm <- lm(tadpoles ~ frogs+frogs*spots, data=dat)
tfcclm <- lm(tadpoles ~ frogs+frogs*color, data=dat)
tfsscclm <- lm(tadpoles ~ frogs + frogs*spots + frogs*color 
               + spots*color, data=dat)
tffsscclm <- lm(tadpoles ~ frogs + spots + color + frogs*spots
                + frogs*color + spots*color, data=dat)

summary(tffsscclm)

#obviously only frogs will be significant when the other data is boolean or character???... can't do an lm with that
...
```{r}
plotfx <- function(x, y, title, axis1, axis2, pchx, color){
  plot(x, y,
       col=as.character(color),
       pch=pchx,
       xlab= as.character(axis1),
       ylab= as.character(axis2),
       main= as.character(title)
       )
}

#data subset tads>5:
subdat <- dat[dat$tadpoles>5,]

#plot
plotfx(subdat$frogs, subdat$tadpoles, 'Frogs Plot', 'frogs', 'tadpoles', 16, 'blue')
