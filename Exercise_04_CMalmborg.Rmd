...
```{r}
#maturation data plot:
plot(dia,cones, col='brown', pch=2)

#params vector:
param <- c(mean(dia), sd(dia))

#adding theta:
tseq = 1:max(dia)
lines(tseq,pnorm(tseq,param[1],param[2]),type='l')
...
```{r}
##' @param param   parameter vector (b0,b1)
##' @param m    tree maturity status
##' @param x   tree diameter
likMat = function(param,x,m){
  theta = pnorm(x,param[1],param[2])
  -sum(dbinom(m, 1, theta,log=TRUE))  #negative log likelihood
}
```
...
```{r}
out.mat <- optim(param,likMat, x=dia, m=cones)
out.mat

plot(dia,cones, col='brown', pch=2)
lines(tseq,pnorm(tseq,out.mat$par[1],out.mat$par[2]),type='l')
...
plot(dia, ncone, col='dark green', pch=20)
lines(dseq,a0*dseq^2,col=3,lwd=3)

plot(dia,cones, col='brown', pch=2)
lines(tseq,pnorm(tseq,out$par[2],out$par[3]),type='l')
```
...
paramtable<-cbind(a0,b0,b1,lnL,n)
rownames(paramtable)<-c('all','amb','elev')
print(paramtable)
...
#Model plot:
plot(dia, ncone, col='dark green', pch=20, 
main='Number of Cones by Tree Size', xlab='Tree Diameter', ylab='Number of Cones')
lines(dseq,a0[1]*dseq^2,col="green",lwd=3)
lines(dseq,a0[2]*dseq^2,col="blue",lwd=3)
lines(dseq,a0[3]*dseq^2,col="purple",lwd=3)
legend("topleft",
       legend=c("Full model","Ambient model","Elevated CO2 model"),
       col=c('green','blue','purple'),
       lty=1)


plot(dia,cones, col='brown', pch=2,
     main='Cones Produced by Tree Size', xlab='Tree Diameter', ylab='Probability of Cones Produced')
lines(tseq,pnorm(tseq,out$par[2],out$par[3]),
      type='l', col='green', lwd=2)
lines(tseq,pnorm(tseq,out.amb$par[2],out.amb$par[3]),
      type='l', col='blue', lwd=2)
lines(tseq,pnorm(tseq,out.elev$par[2],out.elev$par[3]),
...
> Yes, the results from these models suggest that elevated CO2 does increases fecundity. We can see in the first plot above that the number of cones produced is higher for the elevated CO2 treatment compared with the combined and ambient CO2 models.
...
> Upon visual inspection of the second plot above we can see that there are more likely to be cones produced at smaller diameters with the elevated CO2 model compared with the ambient CO2 model or combined data model.
