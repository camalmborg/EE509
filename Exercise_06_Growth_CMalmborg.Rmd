...
While initially one may object to the choice of the Normal based on the argument that growth must be positive, in fact trees can genuinely lose height due to dieback or can appear to lose height due to measurement error.
...
mseq<-seq(0,max(L), length.out=130)

#A: Plot the data:
plot(L, grow, main='Tree Growth with Light', xlab='Proportion of Light', 
     ylab='Growth Rate', col='green', pch=20)
#B: Add line:
lines(mseq,beta[1]+beta[2]*(mseq/(theta + mseq)), 
...
This conditional posterior is based on selecting the terms from the full posterior (above) that include theta.  Since this clearly is a non-standard distribution we will sample from it using Metropolis-Hastings.  We will be using the Metropolis-Hastings algorithm rather than the simpler Metropolis because our truncated Normal distribution is non-symmetric.
...
Finally, we need to put all the pieces together and save the results in a “storage” section of the MCMC 
...
##putting the variables here if we want to change them:
##storage for MCMC
ngibbs <- 12000    			## number of updates
bgibbs <- matrix(0.0,nrow=ngibbs,ncol=2) 	## storage for beta
sgibbs <- rep(sg,ngibbs)			## storage for sigma2
tgibbs <- rep(theta,ngibbs)   ## storage for theta
JumpSD <- 0.025
...
for(g in 1:ngibbs){
  ...
  if(g %%2000 == 0) print(g)	##counter to show how many steps have been performed
}
```
...
```{r}
#library needed for calculating burn-in with only 1 chain:
library(LaplacesDemon)

#burn in:
burnin(out)
```
> At both 8,000 runs in the chain I got a burn-in of 4,000. Since that was half of the convergence time for the 8,000 run I increased to 10,000 iterations, ending with the same burn-in. At 10,000 iterations the highest effective sample size was for s at ~7,100 runs. I decided to up it to 15,000 iterations as a final number. At that point burnin was 0 and effective sample size of s was ~12,000. My final run settled on 12,000 iterations, burn in of 0 again, effective sample size of 9,788 for s.
...
> JumpSD Values (with sample size 8000 runs):
0.01, Acceptance rate: 71%
0.05, Acceptance rate: 27%
0.025, Acceptance rate: 46% ding ding ding we have a winner!
...
mich_ment <- "model{
  ...
  for(i in 1:n){
      mu[i] <- b[1] + b[2]*(L[i]/(theta + L[i]))     ## process model
...
#THE DATA:
data <- list(L = L, y = grow, n = length(grow))

#THE PRIORS:
data$b0 <- as.vector(c(0,0))
data$Vb <- solve(diag(12000,2))
data$s1 <- 0.1
data$s2 <- 0.1
data$theta0 <- 0.1  ### add initial value of theta
# 
...
#RUN IT, YOU JAG OFF:
j.model <- jags.model (file = textConnection(mich_ment),
                             data = data,
                             inits = inits,
                             n.chains = 3)
...
for(g in 1:ngibbs){
  Ey <- jout[g,2] + jout[g,3] * xpred/(xpred + jout[g,4])
  ycred[g,] <- Ey
  ypred[g,] <- rnorm(npred,Ey,(1/(sqrt(jout[g,1]))))
}
ci <- apply(ycred,2,quantile,c(0.025,0.5,0.975))
pi <- apply(ypred,2,quantile,c(0.025,0.975))
...
> One major difference between the outputs is that the JAGS code operates using precisions rather than SE's, so the S estimates for the MCMC run with JAGS vs the manual MCMC appear very different when they produce similar results when graphed, correcting for this difference. 
The b1 and b2 parameters tended to be slightly lower for the manual MCMC, but not largely different from the JAGS model. The theta values estimated in the model were essentially the same.
The plots show that the estimated values with manual and JAGS MCMC's are similar, in both cases only 3 points on the graph appear to fall outside the predictive intervals, and the CI's look similar. Overall, both versions of the MCMC are reasonably able to estimate the parameters for this model.
