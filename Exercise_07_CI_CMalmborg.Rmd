...
```{r}
aic.amb <- (-2*out.amb$value)+(2*(length(out.amb$par)))
print(aic.amb)
aic.elev <- (-2*out.elev$value)+(2*(length(out.elev$par)))
print(aic.elev)
```
...
## density plot b0
plot(density(mle[,2],width=5),type='l')
abline(v=b0.ci,lty=2)
abline(v=b0[2])

## density plot b1
plot(density(mle[,3],width=5),type='l')
...
print("parameter sd's:")
apply(mle,2,sd)

print("parameter quantiles:")
apply(mle,2,quantile,c(0.025,0.975))
```
...
for(i in 1:nboot){
  conf.mat[i,]  <- pnorm(dseq,mle[i,2],mle[i,3])        ## store model | params
...
```{r}
for(i in 1:nboot){
  conf.cone[i,]  <- mle[i,1]*dseq^2       ## store model | params
  pred.cone[i,]  <- rlnorm(length(dseq),log(mle[i,1]),conf.cone[i,])
}
...
npred <- 31				## number of X values you predict for
dseq  <- seq(0,30,length=npred)		## diameter sequence
mle.el   <- matrix(NA,nrow=nboot,ncol=3)	## storage for parameter estimates
conf.mat.el  <- matrix(NA,nrow=nboot,ncol=npred)	## storage for maturation CI
conf.cone.el <- matrix(NA,nrow=nboot,ncol=npred)	## storage for fecundity CI
pred.mat.el   <- matrix(NA,nrow=nboot,ncol=npred)	## storage for predictive maturation
pred.cone.el <- matrix(NA,nrow=nboot,ncol=npred)	## storage for predictive fecundity
...
#running model with elev treatment:
for(i in 1:nboot){
  if(i%%100 == 0) print(i)  					## progress indicator
  samp <- sample(length(dia.t),replace=T)		##Sample row indices 
  out.boot <- optim(param,likfit,method="L-BFGS-B",	## fit model to sample
                    lower=c(0.001,10,1),upper=c(1,30,30),dia=dia.t[samp],ncone=ncone.t[samp])
  mle.el[i,] <- out.boot$par					## store parameters
}

#pairs plot:
colnames(mle.el) = c("a0","b0","b1")
a0.ci.el <- quantile(mle.el[,1],c(0.025,0.975))
b0.ci.el <- quantile(mle.el[,2],c(0.025,0.975))
b1.ci.el <- quantile(mle.el[,3],c(0.025,0.975))
pairs(mle.el)

#parameter density plots:
## density plot a0
plot(density(mle.el[,1],width=0.005),type='l',xlim=c(0,0.05),ylim=c(0,120))
abline(v=a0.ci.el,lty=2)
abline(v=a0[2])

## density plot b0
plot(density(mle.el[,2],width=5),type='l')
abline(v=b0.ci.el,lty=2)
abline(v=b0[2])

## density plot b1
plot(density(mle.el[,3],width=5),type='l')
abline(v=b1.ci.el,lty=2)
abline(v=b1[2])
...
print("parameter quantiles:")
apply(mle.el,2,quantile,c(0.025,0.975))

#confidence intervals:
for(i in 1:nboot){
  conf.mat.el[i,]  <- pnorm(dseq,mle.el[i,2],mle.el[i,3])        ## store model | params
  pred.mat.el[i,]  <- rbinom(npred,1,conf.mat.el[i,])
}

ci.mat.el <- apply(conf.mat.el,2,quantile,c(0.025,0.5,0.975))
pi.mat.el <- apply(pred.mat.el,2,quantile,c(0.025,0.975))

## maturation plot
cones = b$c00 > 0
plot(b$diam,cones,col = b$tmt,ylim=c(0,1))
lines(dseq,ci.mat.el[2,],col=2,lwd=3)   ## median model
lines(dseq,ci.mat.el[1,],col=2,lty=2,lwd=3)	## 95% CI
lines(dseq,ci.mat.el[3,],col=2,lty=2,lwd=3)
lines(dseq,pi.mat.el[1,],col=3,lty=2,lwd=3)	## 95% PI
lines(dseq,pi.mat.el[2,],col=3,lty=2,lwd=3)

#confidence intervals:
for(i in 1:nboot){
  conf.cone.el[i,]  <- mle.el[i,1]*dseq^2       ## store model | params
  pred.cone.el[i,]  <- rlnorm(length(dseq),log(mle.el[i,1]),conf.cone.el[i,])
}

ci.cone.el <- apply(conf.cone.el,2,quantile,c(0.025,0.5,0.975))
pi.cone.el <- apply(pred.cone.el,2,quantile,c(0.025,0.975)) 

plot(dia,ncone)
lines(dseq,ci.cone.el[2,],col=2,lwd=3)   ## median model
lines(dseq,ci.cone.el[1,],col=2,lty=2,lwd=3)	## 95% CI
lines(dseq,ci.cone.el[3,],col=2,lty=2,lwd=3)
lines(dseq,pi.cone.el[1,],col=3,lty=2,lwd=3)	## 95% PI
lines(dseq,pi.cone.el[2,],col=3,lty=2,lwd=3)
...
lines(dseq,ci.mat[2,],col=4,lwd=3)   ## median model
lines(dseq,ci.mat[1,],col=4,lty=2,lwd=3)	## 95% CI
lines(dseq,ci.mat[3,],col=4,lty=2,lwd=3)
lines(dseq,pi.mat[1,],col=5,lty=2,lwd=3)	## 95% PI
lines(dseq,pi.mat[2,],col=5,lty=2,lwd=3)
...
lines(dseq,ci.cone[2,],col=4,lwd=3)   ## median model
lines(dseq,ci.cone[1,],col=4,lty=2,lwd=3)	## 95% CI
lines(dseq,ci.cone[3,],col=4,lty=2,lwd=3)
lines(dseq,pi.cone[1,],col=5,lty=2,lwd=3)	## 95% PI
lines(dseq,pi.cone[2,],col=5,lty=2,lwd=3)
